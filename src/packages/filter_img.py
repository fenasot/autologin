...
class filter_img:
    ...
    def resize_img(self, scale_factor=5):
        ...
        self.image = cv2.resize(
            self.image, (width, height), interpolation=cv2.INTER_LINEAR)
        return self
    ...
    def gray_img_2(self):
        gray_image = cv2.cvtColor(self.image, cv2.COLOR_BGR2GRAY)
        _, self.image = cv2.threshold(
            gray_image, 200, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
        return self
    ...
    def gray_img_3(self):
        gray_image = cv2.cvtColor(self.image, cv2.COLOR_BGR2GRAY)
        _, self.image = cv2.threshold(
            gray_image, 250, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
        return self
